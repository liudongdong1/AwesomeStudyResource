...
(function() {
  var __slice = [].slice;
  var cached_voc_data = 0; //列表数据是否已经加载过一次。用于当用户在编辑器输入@符号之后再加载，而不是使用原来的注册事件的时候就加载用户列表。--by fanghai_li@163.com
  ...
  (function(factory) {
    ...
    Model = (function() {
      ...
      Model.prototype.load = function(data) {
    	cached_voc_data = 1; //同一次操作，避免2次发送ajax请求
...
//        this.model.load(current_settings.data);//加载请求的数据
        return this;
      };
      ...
      Controller.prototype.on_keydown = function(e) {
		if(!cached_voc_data){	//监听到编辑器有键盘事件的时候加载数据，是否已经加载过一次
    ...
    $.fn.atwho = function(flag, options) {
      return this.filter('textarea, input').each(function() {
        ...
        return data.reg(flag, options);//绑定编辑器，注册监听事件
      });
    };
